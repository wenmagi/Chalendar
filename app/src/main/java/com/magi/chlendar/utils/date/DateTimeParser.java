...
final class DateTimeParser {
    ...
    private static final Integer NUM_DIGITS = parseInt(NUM_DIGITS_FOR_FRACTIONAL_SECONDS);
    ...
    private void parseTime(String aTime) {
        ...
        if (matcher.matches()) {
            ...
            if (minute != null) {
                fMinute = parseInt(minute);
            }
            String second = getGroup(matcher, 3, 7);
            if (second != null) {
                fSecond = parseInt(second);
            }
            String decimalSeconds = getGroup(matcher, 4);
            if (decimalSeconds != null) {
                fNanosecond = parseInt(convertToNanoseconds(decimalSeconds));
            }
        } else {
